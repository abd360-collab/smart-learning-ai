...
const QuizResultPage = () => {
  ...
  const correctAnswers = detailedResults.filter(
    r => normalize(r.selectedAnswer) === normalize(r.correctAnswer)
  ).length;
  ...
  return (
    <div className='max-w-5xl mx-auto px-4 py-8'>
      ...
      <div className='space-y-6'>
        ...
        {detailedResults.map((result, index) => {
          const userAnswerIndex = result.options.findIndex(
            opt => normalize(opt) === normalize(result.selectedAnswer)
          );

          const correctAnswerIndex = result.options.findIndex(
            opt => normalize(opt) === normalize(result.correctAnswer)
          );

          const isCorrect =
            normalize(result.selectedAnswer) === normalize(result.correctAnswer);
          ...
          return (
            <div key={index} className='bg-white border-2 border-slate-200 rounded-2xl p-6 shadow-sm'>
              ...
              <div className='flex items-start justify-between gap-4 mb-4'>
                ...
                <div className={`w-10 h-10 flex items-center justify-center rounded-xl border-2 ${
                  isCorrect ? "bg-emerald-50 border-emerald-200" : "bg-rose-50 border-rose-200"
                }`}>
                  {isCorrect ? (
                  ...
                  return (
                    ...
                    >
                      <div className='flex justify-between items-center'>
                        ...
                        {isCorrectOption && (
                          <span className='text-xs bg-emerald-200 px-2 py-0.5 rounded'>
                            Correct
                          </span>
                        )}

                        {isWrongSelection && (
                          <span className='text-xs bg-rose-200 px-2 py-0.5 rounded'>
                            Your Answer
                          </span>
                        )}
                      </div>
      ...
      <div className="mt-8 flex justify-center">
        <Link to={`/documents/${quiz?.document?._id}`}>
          <button className="px-8 h-12 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl">
            Return to Document
          </button>
        </Link>
      </div>
